...
class RemoteLock:
            ...
            for g in r:
                ...
                if department and department.startswith('[{'):
                    ...
                    target_slots, exception_slots = self.make_calendar_list(
                        access_info_list=deptjson, start_day=start_day, day_range=target_day_range, exp_day_range=exp_day_range)
                    ret.append({
                        'type': 'access_user',
                        'id': g['id'],
    ...
    def get_access_guests(self, target_year: int, target_month: int) -> list[dict]:
        ret = []
        ret.extend(self.__get_access_guests(
            'expired', target_year, target_month))
        ret.extend(self.__get_access_guests(
            'current', target_year, target_month))
        ret.extend(self.__get_access_guests(
            'upcoming', target_year, target_month))
        return ret

    def __get_access_guests(self, status: str, target_year: int, target_month: int) -> list[dict]:
        end_of_read: bool = False
        ret = []
        page: int = 1
        while not end_of_read:
    ...
    def make_access_guest_data(self, item) -> dict:
        ...
        s_datetime = datetime(int(s_year), int(
            s_month), int(s_day), int(s_hour), int(s_min), int(s_sec))
        e_datetime = datetime(int(e_year), int(
            e_month), int(e_day), int(e_hour), int(e_min), int(e_sec))

        slot1 = datetime(int(s_year), int(s_month),
                         int(s_day), 7, 0, 0)  # 早朝枠 5:00-9:00
        slot2 = datetime(int(s_year), int(s_month),
                         int(s_day), 10, 0, 0)  # 午前枠 9:00-13:00
        slot3 = datetime(int(s_year), int(s_month),
                         int(s_day), 15, 0, 0)  # 午後枠 13:00-17:00
        slot4 = datetime(int(s_year), int(s_month),
                         int(s_day), 19, 0, 0)  # 夜枠 17:00-21:00
    ...
    def transform_rsv_time(self):
        ...
        m = re.match(
            r'([0-9]+)/([0-9]+)/([0-9]+) ([0-9]+):([0-9]+)[^0-9]+([0-9]+):([0-9]+)', rsvtime)
    ...
    def api(self, path: str, params: dict[str, Any] = {}, method='POST', with_metadata: bool = False) -> dict[str, Any]:
        ...
        if method == 'POST':
            ...
            if r.status_code == 409:
                logger.warn({'service': 'remotelock',
                            'cause': 'duplication error', 'params': params})
    ...
    def __refresh_token(self, remotelock_token: dict[str, str]) -> dict[str, str]:
        ...
        client_key = parameters.get_parameter(
            'remotelock_clientkey', transform="json")
    ...
    def make_calendar_list(self, access_info_list: list[dict], start_day: datetime, day_range: int, exp_day_range: int = 365):
        weekday_dict = {'Mon': 0, 'Tue': 1, 'Wed': 2,
                ...
                if target_wd == wd:
                    if no in access_info['week']:
                        ...
                        while len(slot_queue) > 0:  # 同一日複数予約に対応するためのループ
                            ...
                            target_list.append(
                                {"day": dtstr, "start_time": start_dt, "end_time": end_dt, "start_time_iso": start_iso, "end_time_iso": end_iso})
                    else:
